...
void moveForward(){
  ...
  leftMotor.forward(currSpeed);
  rightMotor.forward(currSpeed);
}

void moveBackward(){
  negateOtherEnc = false;
  currSpeed = nextSpeed(MOVE_SPEED_MIN, MOVE_SPEED_MAX);
  leftMotor.backward(currSpeed);
  rightMotor.backward(currSpeed);
}
...
void handleOdometry(unsigned long time){
  double vel_l = 0;
  double vel_r = 0;
  long totalCoder0 = leftEncoder.getEncoderCount();
